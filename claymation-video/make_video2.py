...
SKIN_D = (94, 62, 42)
WRAP = (214, 96, 41)        # headwrap orange
BLAZER = (32, 120, 118)     # teal blazer
GOLD = (233, 178, 70)
...
def s1(d, k, n):
    ...
    clay_text(d, "Nonprofits. Agencies. Advocates.", W // 2, 80, F44, DEEP, k)
    if t > 0.4:
        clay_text(d, "Drowning in messy data — with communities counting on you.", W // 2, 150, F26, (120, 60, 40), k)
scene(6, s1)
...
def s3(d, k, n):
    ...
    clay_text(d, "Clean data. Clear insight. Real impact.", W // 2, 80, F44, DEEP, k)
    if t > 0.6:
        clay_text(d, "Dynamics 365 · Power Platform · Dashboards · Data care", W // 2, 150, F26, PURPLE, k)
scene(6, s3)

# Scene 4 (8s): offers & pricing cards stamp in
OFFERS = [
    ("Data Health Check", "$499", "Audit of your data,\nsystems & reports", TEAL),
    ("Data Rescue", "from $1,500", "Clean-up, migration &\none source of truth", ORANGE),
    ("CRM & Automation", "from $3,500", "Dynamics 365 / Power\nPlatform built for you", PURPLE),
    ("Ongoing Care", "$299/mo", "Support, training &\nmonthly insights", DEEP),
]
def s4(d, k, n):
    t = k / n
    clay_text(d, "How we help — nonprofit-friendly pricing", W // 2, 70, F44, DEEP, k)
    cw, ch, gap = 275, 330, 30
    x0 = (W - 4 * cw - 3 * gap) // 2
...
def s5(d, k, n):
    t = k / n
    items = [("BData Solutions", F64, TEAL, 190, 0.05),
             ("Data with heart, for missions that matter.", F26, PURPLE, 270, 0.22),
             ("www.barakabelle.ca", F44, DEEP, 360, 0.38),
             ("hello@barakabelle.ca", F34, ORANGE, 440, 0.55),
             ("hellobdatasolutions@gmail.com", F26, (120, 90, 60), 500, 0.68)]
...
out = "/tmp/claude-0/-home-user-BDataSolutions/e8b5f2ae-3431-5d77-9233-5ca80f105b1a/scratchpad/bdata-claymation-campaign.mp4"
